package fr.drogonistudio.packets_listener.api.reflective;
...
public final class NmsReflection
{
    ...
    private static Method getMethodFromArrayAndIndex(Method[] methods, Class<?> returnType, Class<?>[] parametersType, int index) throws NoSuchMethodException
    {
        ...
        for (Method method : methods) {
            if (method.getReturnType().getName().equals(returnType.getName()) && Arrays.equals(method.getParameterTypes(), parametersType)) {
                if (counter == index)
                    return method;
                else
                    counter++;
    ...
    static
    {
        ...
        Logger pluginLogger = SimplePacketListenerPlugin.getInstance().getLogger();
